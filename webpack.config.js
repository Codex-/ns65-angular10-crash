	...
	const config = {
		...
		plugins: [
			...
			new webpack.DefinePlugin({
				'global.ENV_PRODUCTION': !!env.production,
			...
			// Fix __dirname by modifying the variable to be __dirname (path of the bundle file at runtime) and the remaining relative path
			{
				apply(compiler) {
					...
					compiler.plugin('compilation', function (compilation, params) {
						params.normalModuleFactory.plugin('parser', function (parser) {
							parser.plugin('expression __dirname', function () {
								var relativePath = relative(context, this.state.module.context);
								if (relativePath.indexOf('modules') !== -1) {
								...
								if (relativePath.indexOf('node_modules') === -1) {
									// Prevent rewriting __dirname in node_modules
